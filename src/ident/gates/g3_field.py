...
def _reconstruct_frames(dump, pressure_mode="true", quad_dx_factor=1.0,
    ...
    for fi in range(0, meta.n_frames, frame_stride):
        ...
        else:  # P1 closure from the reconstructed field's vertical accel proxy
            # approximate a_z on the grid via field advective term L@v (dv/dt added later if needed)
            a_z = np.einsum("nij,nj->ni", D_g * 0.0 + fld.grad_v(grid), v_g)[:, 1]
            p_g = p1_particles(grid[:, 0], grid[:, 1], a_z,
                               np.full(grid.shape[0], rho_bulk), 4 * dx)
        gd = equivalent_shear_rate(D_g, EPS_GAMMA_DEFAULT)
        ...
        frames_out.append(FrameData(
            t=fr.t, x=grid, v=v_g, D=D_g,
            a=np.zeros_like(v_g),  # time-weak form does not use a
...
def run_gate(dump_path, out_dir="out/g3", dictionary=None, pressure_mode="true",
    ...
    result = {
        ...
        "observed_I": list(bundle.I_observed),
        "theta_hat_timeweak": res_tw.theta.tolist(),
        "cond": res_tw.cond_AtA,
    }
    meta = dump.meta
    Igrid = np.logspace(np.log10(max(bundle.I_observed[0], 1e-3)),
                        np.log10(max(bundle.I_observed[1], 1e-2)), 80)
